...
async def suggest(req: ScenarioSuggestionRequest) -> list[SuggestedAction]:
    """Suggests interventions from the rule base, refined by the anomaly detector."""
    ...
    if not acts and req.recent_metrics:
        anomalies = await detect(AnomalyRequest(metrics=req.recent_metrics))
        if anomalies:
            acts.append(SuggestedAction(
                action="REVIEW_ALERT", domain="global",
                rationale=f"rolling anomaly detected at index {anomalies[-1].index}"))
    return acts